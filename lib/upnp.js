...
ControlPoint.prototype.onResponseMessage = function(msg, rinfo){
  ...
  this.responseParser.reinitialize(HTTPParser.RESPONSE);	// reinitialise the HTTP parser
...
ControlPoint.prototype._initParsers = function() {
  ...
  if (!self.requestParser) {
    self.requestParser = http_common.parsers.alloc();
    self.requestParser.reinitialize(HTTPParser.REQUEST);
    self.requestParser.onIncoming = function(req) { };
    self.requestParser.socket = { }; //fix for issue #3
  }
  if (!self.responseParser) {
    self.responseParser = http_common.parsers.alloc();
    self.responseParser.reinitialize(HTTPParser.RESPONSE);
    console.log(Object.keys(HTTPParser));
    self.responseParser.onIncoming = function(req) { };
    self.responseParser.socket = { }; //fix for issue #3
  }
};
